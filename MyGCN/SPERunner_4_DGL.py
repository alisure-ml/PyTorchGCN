...
class EmbeddingNetCIFARSmallNorm(nn.Module):
    ...
    def __init__(self, has_sigmoid=True, is_train=True):
        ...
        # input 24
        self.conv11 = ConvBlock(3, 64, has_bn=self.has_bn)  # 8
        self.conv12 = ConvBlock(64, 64, has_bn=self.has_bn)  # 8
        self.pool1 = nn.MaxPool2d(2, 2)  # 4
        self.conv21 = ConvBlock(64, 64, has_bn=self.has_bn)  # 4
        self.conv22 = ConvBlock(64, 64, has_bn=self.has_bn)  # 4
        self.pool2 = nn.MaxPool2d(2, 2)  # 4
        self.conv31 = ConvBlock(64, 128, has_bn=self.has_bn)  # 2
        self.conv32 = ConvBlock(128, 128, has_bn=self.has_bn)  # 2
        self.pool3 = nn.MaxPool2d(2, 2)  # 1

        self.conv_shape1 = ConvBlock(128, 128, padding=0, ks=1, has_bn=self.has_bn)
        self.conv_shape2 = ConvBlock(128, self.embedding_size, padding=0, ks=1, has_bn=self.has_bn, bias=False)
        self.conv_texture1 = ConvBlock(128, 128, padding=0, ks=1, has_bn=self.has_bn, bias=self.has_bn)
        self.conv_texture2 = ConvBlock(128, self.embedding_size, padding=0, ks=1, has_bn=self.has_bn, bias=False)

        self.shape_conv1 = ConvBlock(self.embedding_size, 32, padding=0, ks=1, has_bn=self.has_bn)  # 1
        self.shape_up1 = nn.UpsamplingBilinear2d(scale_factor=2)  # 2
        self.shape_conv2 = ConvBlock(32, 32, has_bn=self.has_bn)  # 2
        self.shape_up2 = nn.UpsamplingBilinear2d(scale_factor=2)  # 4
        self.shape_conv3 = ConvBlock(32, 32, has_bn=self.has_bn)  # 4
        self.shape_up3 = nn.UpsamplingBilinear2d(scale_factor=2)  # 8
        self.shape_conv4 = ConvBlock(32, 32, has_bn=self.has_bn)  # 8
        self.shape_out = ConvBlock(32, 1, has_relu=False, has_bn=self.has_bn)  # 8

        self.texture_conv1 = ConvBlock(self.embedding_size * 2, 128, padding=0, ks=1, has_bn=self.has_bn)  # 1
        self.texture_up1 = nn.UpsamplingBilinear2d(scale_factor=2) # 2
        self.texture_conv21 = ConvBlock(128, 128, has_bn=self.has_bn)  # 2
        self.texture_conv22 = ConvBlock(128, 128, has_bn=self.has_bn)  # 2
        self.texture_up2 = nn.UpsamplingBilinear2d(scale_factor=2)  # 4
        self.texture_conv31 = ConvBlock(128, 64, has_bn=self.has_bn)  # 4
        self.texture_conv32 = ConvBlock(64, 64, has_bn=self.has_bn)  # 4
        self.texture_up3 = nn.UpsamplingBilinear2d(scale_factor=2)  # 8
        self.texture_conv41 = ConvBlock(64, 64, has_bn=self.has_bn)  # 8
        self.texture_conv42 = ConvBlock(64, 64, has_bn=self.has_bn)  # 8
        self.texture_out = ConvBlock(64, 3, has_relu=False, has_bn=self.has_bn)  # 8
    ...
    def forward_train(self, x):
        ...
        e2 = self.pool2(self.conv22(self.conv21(e1)))
        e3 = self.pool3(self.conv32(self.conv31(e2)))

        shape_norm = self.norm(self.conv_shape2(self.conv_shape1(e3)))
        texture_norm = self.norm(self.conv_texture2(self.conv_texture1(e3)))
        ...
        shape_out = self.shape_out(self.shape_conv4(shape_d3))
        ...
        texture_d3 = self.texture_up3(self.texture_conv32(self.texture_conv31(texture_d2)))
        texture_out = self.texture_out(self.texture_conv42(self.texture_conv41(texture_d3)))
