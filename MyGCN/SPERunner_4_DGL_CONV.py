...
class DealSuperPixel(object):
    ...
    def run(self):
        ...
        _region_props = skimage.measure.regionprops(segment + 1)

        sp_info = {}
        for i in range(segment.max() + 1):
            now_region = self._get_region_i(_region_props, i)

            # 坐标 和 是否属于超像素
            _now_where = now_region.coords
            sp_area = now_region.bbox
            sp_label = np.asarray(now_region.image, dtype=np.int)

            # 计算邻接矩阵
            _now_adj = skimage.morphology.dilation(segment == i, selem=skimage.morphology.square(3))
            _sp_adj_id = [sp_id for sp_id in np.unique(segment[_now_adj]) if sp_id != i]
            _now_center = now_region.centroid
            _adj_center = [self._get_region_i(_region_props, _sp_id).centroid for _sp_id in _sp_adj_id]
            _adj_dis = [np.sqrt((_now_center[0] - center[0]) ** 2 +
                                (_now_center[1] - center[1]) ** 2) for center in _adj_center]
            sp_adj = [i, _sp_adj_id, list(self._softmax_of_distance(_adj_dis))]

            # 计算单个超像素中的邻接矩阵
            pixel_adj = []
            for one_i, one in enumerate(_now_where):
                _adj, _w = [], []
                for n_i in range(len(_now_where)):
                    _dis = (one[0] - _now_where[n_i][0]) ** 2 + (one[1] - _now_where[n_i][1]) ** 2
                    if n_i != one_i and _dis <= 2:
                        _adj.append(n_i)
                        _w.append(np.sqrt(_dis))
                        pass
                    pass
                pixel_adj.append([one_i, list(one), _adj, list(self._softmax_of_distance(_w))])
                pass

            # 结果
            sp_info[i] = {"area": sp_area, "label": sp_label, "sp_adj": sp_adj, "pixel_adj": pixel_adj}
            pass

        return segment, sp_info
    ...
    @staticmethod
    def _get_region_i(region_props, i):
        if region_props[i].label == i + 1:
            return region_props[i]
        else:
            for region_one in region_props:
                if region_one.label == i + 1:
                    return region_one
            pass
        Tools.print("Error in _get_region_i ......")
        return None
    ...
    def demo():
        ...
        now_segment, now_super_pixel_info = deal_super_pixel.run()
...
class MyDataset(Dataset):
    ...
    def get_sp_info(self, img):
        ...
        segment, sp_info = deal_super_pixel.run()

        #################################################################################
        # Data
        edge_index, edge_w = [], []
        for sp_i in range(len(sp_info)):
            sp_i_adj = sp_info[sp_i]["sp_adj"]
            for adj_id, adj_w in zip(sp_i_adj[1], sp_i_adj[2]):
                edge_w.append(adj_w)
                edge_index.append([sp_i_adj[0], adj_id])
            pass
        ...
        for sp_i in range(len(sp_info)):
            # Data
            pixel_adj = sp_info[sp_i]["pixel_adj"]
            data_where, edge_index, edge_w = [], [], []
            for pixel_i in pixel_adj:
                data_where.append([0, pixel_i[1][0], pixel_i[1][1]])
                for adj_id, adj_w in zip(pixel_i[2], pixel_i[3]):
                    edge_w.append(adj_w)
                    edge_index.append([pixel_i[0], adj_id])
                pass
            edge_w = np.asarray(edge_w)
            edge_index = np.asarray(edge_index)
            data_where = np.asarray(data_where)

            # Graph
            small_graph = dgl.DGLGraph()
            small_graph.add_nodes(len(pixel_adj))
            small_graph.ndata['data_where'] = torch.from_numpy(data_where).long()
            small_graph.add_edges(edge_index[:, 0], edge_index[:, 1])
            small_graph.edata['feat'] = torch.from_numpy(edge_w).unsqueeze(1).float()
