...
class MyGCNNet(nn.Module):
    ...
    def __init__(self, which=0, has_bn=False, normalize=False, residual=False, improved=False, concat=False):
        super().__init__()
        self.model_conv = CONVNet()  # 14, 20

        if which == 0:
            self.model_gnn1 = GCNNet1(in_dim=self.model_conv.out_num, hidden_dims=[512, 512],
            ...
            self.model_gnn1 = SAGENet1(in_dim=self.model_conv.out_num, hidden_dims=[512, 512],
...
class RunnerSPE(object):
    ...
    def __init__(self, data_root_path='/mnt/4T/Data/cifar/cifar-10', down_ratio=4, concat=False, which=0,
        ...
        self.model = MyGCNNet(which=which, has_bn=has_bn, normalize=normalize,
                              residual=residual, improved=improved, concat=concat)
...
if __name__ == '__main__':
    ...
    SAGENet    1_4_4_MoreConv 21264960 2020-08-12 08:56:35 Epoch:14, Train:0.6917-0.8906/1.2917 Test:0.6758-0.8859/1.3281
    """
    ...
    _root_ckpt_dir = "./ckpt2/dgl/1_PYG_CONV_Fast-ImageNet/{}_{}_{}_MoreConv".format(_which, _sp_size, _down_ratio)

    Tools.print("epochs:{} ckpt:{} batch size:{} image size:{} sp size:{} down_ratio:{} "
                "workers:{} gpu:{} has_residual:{} is_normalize:{} "
                "has_bn:{} improved:{} is_sgd:{} weight_decay:{} concat:{} which:{}".format(
        _epochs, _root_ckpt_dir, _batch_size, _image_size, _sp_size, _down_ratio, _num_workers,
