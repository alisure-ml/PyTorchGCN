...
class CNNNet2(nn.Module):
    ...
    def __init__(self, in_dim=128, hidden_dims=[128, ], has_bn=False, normalize=False, residual=False, improved=False):
        super().__init__()
        self.hidden_dims = hidden_dims
        self.normalize = normalize
        self.residual = residual
...
class SAGENet2(nn.Module):
    ...
    def __init__(self, in_dim=128, hidden_dims=[128, ], has_bn=False, normalize=False, residual=False, concat=False):
        super().__init__()
        self.hidden_dims = hidden_dims
        self.normalize = normalize
        self.residual = residual
...
class GATNet2(nn.Module):
    ...
    def __init__(self, in_dim=128, hidden_dims=[128, ], has_bn=False, residual=False, concat=False, heads=8):
        super().__init__()
        self.hidden_dims = hidden_dims
        self.residual = residual
        self.has_bn = has_bn
...
class ResGatedGCN2(nn.Module):
    ...
    def __init__(self, in_dim=128, hidden_dims=[128, 128, 128, 128], has_bn=False, normalize=False, residual=False):
        super().__init__()
        self.hidden_dims = hidden_dims
        self.residual = residual
        self.normalize = normalize
...
class AttentionClass1(nn.Module):
...
class MyGCNNet(nn.Module):
    ...
    def __init__(self, conv_layer_num=6, which=0, has_bn=False,
        ...
        if which == 0:
            ...
            # self.model_gnn2 = GCNNet2(in_dim=self.model_gnn1.hidden_dims[-1], hidden_dims=[128, 128, 128],
            #                           has_bn=has_bn, normalize=normalize, residual=residual, improved=improved)
            ...
            # self.model_gnn1 = GCNNet1(in_dim=self.model_conv.features[-2].num_features, hidden_dims=[128, 128],
            #                           has_bn=has_bn, normalize=normalize, residual=residual, improved=improved)
            # self.model_gnn2 = GCNNet2(in_dim=self.model_gnn1.hidden_dims[-1], dims=[256, 256, 256, 256],
            #                           has_bn=has_bn, normalize=normalize, residual=residual, improved=improved)
            ...
            # self.model_gnn1 = GCNNet1(in_dim=self.model_conv.features[-2].num_features, hidden_dims=[128, 128],
            #                           has_bn=has_bn, normalize=normalize, residual=residual, improved=improved)
            # self.model_gnn2 = GCNNet2(in_dim=self.model_gnn1.hidden_dims[-1], hidden_dims=[256, 256, 512, 512],
            #                           has_bn=has_bn, normalize=normalize, residual=residual, improved=improved)

            # 170112
            # self.model_gnn1 = GCNNet1(in_dim=self.model_conv.features[-2].num_features, hidden_dims=[128, 128],
            #                           has_bn=has_bn, normalize=normalize, residual=residual, improved=improved)
            # self.model_gnn2 = GCNNet2(in_dim=self.model_gnn1.hidden_dims[-1], hidden_dims=[128, 128, 128, 128, 128],
            #                           has_bn=has_bn, normalize=normalize, residual=residual, improved=improved)

            # 203648
            # self.model_gnn1 = GCNNet1(in_dim=self.model_conv.features[-2].num_features, hidden_dims=[128, 128, 128],
            #                           has_bn=has_bn, normalize=normalize, residual=residual, improved=improved)
...
"""
0_True_4_1_6_mean_mean_pool     2020-08-29 01:59:34 Epoch:143, Train:0.9430-0.9995/0.1716 Test:0.8625-0.9940/0.4241
0_True_4_1_6_mean_mean_pool Att 2020-08-29 02:00:44 Epoch:143, Train:0.9527-0.9996/0.1454 Test:0.8716-0.9953/0.4002
0_True_2_2_13_mean_mean_pool     Epoch:104, Train:0.9413-0.9990/0.1746 Test:0.8971-0.9964/0.3169
0_True_2_2_13_mean_mean_pool Att Epoch:144, Train:0.9814-1.0000/0.0598 Test:0.9044-0.9955/0.3435

1_True_4_1_6_mean_mean_pool     2020-08-29 02:04:33 Epoch:145, Train:0.9594-0.9998/0.1223 Test:0.8812-0.9965/0.3735
1_True_4_1_6_mean_mean_pool Att 2020-08-29 01:45:03 Epoch:141, Train:0.9653-0.9998/0.1100 Test:0.8860-0.9959/0.3721
1_True_2_2_13_mean_mean_pool     Epoch:134, Train:0.9751-0.9998/0.0754 Test:0.9063-0.9965/0.3305
1_True_2_2_13_mean_mean_pool Att Epoch:111, Train:0.9684-0.9998/0.0950 Test:0.9089-0.9967/0.3060

2_True_4_1_6_mean_mean_pool     2020-08-29 19:34:34 Epoch:144, Train:0.9528-0.9997/0.1447 Test:0.8675-0.9954/0.4109
2_True_4_1_6_mean_mean_pool Att 2020-08-30 05:05:16 Epoch:139, Train:0.9586-0.9998/0.1273 Test:0.8801-0.9956/0.3895
2_True_2_2_13_mean_mean_pool     2020-08-29 20:55:39 Epoch:149, Train:0.9866-1.0000/0.0489 Test:0.9152-0.9989/0.2790
2_True_2_2_13_mean_mean_pool Att 2020-08-29 18:04:25 Epoch:107, Train:0.9708-0.9997/0.0930 Test:0.9156-0.9974/0.2662

3_True_4_1_6_mean_mean_pool 2020-08-30 14:49:45 Epoch:135, Train:0.9625-0.9997/0.1174 Test:0.8893-0.9967/0.3517
3_True_2_2_13_mean_mean_pool     2020-08-31 03:33:36 Epoch:106, Train:0.9540-0.9993/0.1400 Test:0.8994-0.9969/0.3148
3_True_2_2_13_mean_mean_pool Att 2020-08-31 04:56:40 Epoch:134, Train:0.9818-0.9999/0.0615 Test:0.9053-0.9967/0.3323

4_True_4_1_6_mean_mean_pool     2020-08-30 18:54:42 Epoch:132, Train:0.9155-0.9984/0.2592 Test:0.8453-0.9938/0.4568
4_True_4_1_6_mean_mean_pool Att 2020-08-30 19:28:06 Epoch:140, Train:0.9196-0.9989/0.2460 Test:0.8463-0.9937/0.4552
"""
...
if __name__ == '__main__':
    ...
    _root_ckpt_dir = "./ckpt2/dgl/2_PYG_CONV_Fast_CIFAR10/{}_{}_{}_{}_{}_{}".format(
