...
class MyDataset(Dataset):
    ...
    def get_sp_info(self, img, target, is_add_self=False):
        ...
        _now_texture_list = [np.expand_dims(cv2.resize(
            super_pixel_info[key]["texture"] / 255, (self.sp_ve_size, self.sp_ve_size),
            interpolation=cv2.INTER_NEAREST), axis=-1) for key in super_pixel_info]
        net_data = np.transpose(_now_data_list, axes=(0, 3, 1, 2))
        net_shape = np.transpose(_now_shape_list, axes=(0, 3, 1, 2))
        net_texture = np.transpose(_now_texture_list, axes=(0, 3, 1, 2))
        ...
        for sp_i in range(len(super_pixel_info)):
            ...
            _color = super_pixel_info[sp_i]["color"]
        ...
        graph.ndata['texture'] = torch.from_numpy(net_texture).float()
        graph.ndata['color'] = torch.from_numpy(color).float()
...
class EmbeddingNetCIFARSmallNorm(nn.Module):
    ...
    def __init__(self, has_sigmoid=True, is_train=True):
        ...
        self.texture_out = ConvBlock(64, 1, has_relu=False, has_bn=self.has_bn)  # 6
...
class MyGCNNet(nn.Module):
    ...
    def forward(self, graphs, nodes_data, nodes_color, edges_feat, nodes_num_norm_sqrt, edges_num_norm_sqrt):
        shape_feature, texture_feature, shape_out, texture_out = self.ve_model.forward(nodes_data)

        nodes_feat = torch.cat([shape_feature, texture_feature, nodes_color], dim=1)
...
class RunnerSPE(object):
    ...
    def _train_epoch(self, print_freq=100):
        ...
        for i, (batch_graphs, batch_imgs, batch_labels,
            ...
            batch_nodes_texture = batch_graphs.ndata['texture'].to(self.device)  # num x feat
            batch_nodes_color = batch_graphs.ndata['color'].to(self.device)  # num x feat
            ...
            shape_out, texture_out, logits = self.model.forward(batch_graphs, batch_nodes_data,
                                                                batch_nodes_color, batch_edges_feat,
                                                                batch_nodes_num_norm_sqrt, batch_edges_num_norm_sqrt)
            loss, loss_shape, loss_texture, loss_class = self._loss_total(
                shape_out, texture_out, logits, batch_nodes_shape, batch_nodes_texture, batch_labels)
            loss.backward()
            self.optimizer.step()
    ...
    def test(self, print_freq=50):
        ...
        with torch.no_grad():
            for i, (batch_graphs, batch_imgs, batch_labels,
                ...
                shape_o, texture_o, logits = self.model.forward(batch_graphs, batch_nodes_data,
                                                                batch_nodes_color, batch_edges_feat,
                                                                batch_nodes_num_norm_sqrt, batch_edges_num_norm_sqrt)
                loss, loss_shape, loss_texture, loss_class = self._loss_total(
                    shape_o, texture_o, logits, batch_nodes_shape, batch_nodes_texture, batch_labels)
