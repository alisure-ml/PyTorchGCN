...
class DeepPoolLayer1(nn.Module):

    def __init__(self, k, k_out, is_not_last, has_gcn=False, gcn_in=None):
        super(DeepPoolLayer1, self).__init__()
...
class DeepPoolLayer3(nn.Module):

    def __init__(self, k, k_out, is_not_last, has_gcn=False, gcn_in=None):
        super(DeepPoolLayer3, self).__init__()
        self.is_not_last = is_not_last
        self.has_gcn = has_gcn
...
class DeepPoolLayer4(nn.Module):

    def __init__(self, k, k_out, is_not_last, has_gcn=False, gcn_in=None):
        super(DeepPoolLayer4, self).__init__()
        self.is_not_last = is_not_last
        self.has_gcn = has_gcn
...
class MyGCNNet(nn.Module):
    ...
    def __init__(self, has_bn=False, normalize=False, residual=False, concat=True, pool_num=4):
        ...
        if pool_num == 1:
            msa_Layer = DeepPoolLayer1
        elif pool_num == 2:
            msa_Layer = DeepPoolLayer2
        elif pool_num == 3:
            msa_Layer = DeepPoolLayer3
        else:
            msa_Layer = DeepPoolLayer4
            pass

        self.deep_pool4 = msa_Layer(deep_pool[0][0], deep_pool[1][0], True, True, 512)
        self.deep_pool3 = msa_Layer(deep_pool[0][1], deep_pool[1][1], True, True, 512)
        self.deep_pool2 = msa_Layer(deep_pool[0][2], deep_pool[1][2], True, False)
        self.deep_pool1 = msa_Layer(deep_pool[0][3], deep_pool[1][3], False, False)
...
class RunnerSPE(object):
    ...
    def __init__(self, data_root_path, down_ratio=4, sp_size=4, train_print_freq=100, test_print_freq=50,
                 root_ckpt_dir="./ckpt2/norm3", lr=None, num_workers=8, use_gpu=True, gpu_id="1", pool_num=4,
...
"""
ckpt:./ckpt/PYG_GCNAtt_NoAddGCN_NoAttRes_Sigmoid_Abl2468/0
Total param: 42611649 lr_s=[[0, 5e-05], [20, 5e-06]]
2020-09-07 03:06:34 E:29, Train sod-mae-score=0.0087-0.9867 gcn-mae-score=0.0439-0.9171 loss=300.2597(2086.4805+45.8058)
2020-09-07 03:06:34 E:29, Test  sod-mae-score=0.0393-0.8698 gcn-mae-score=0.0756-0.7485 loss=0.3456(0.1810+0.1646)

ckpt:./ckpt/PYG_GCNAtt_NoAddGCN_NoAttRes_Sigmoid_Abl2468/1
Total param: 53523393 lr_s=[[0, 5e-05], [20, 5e-06]]
2020-09-07 03:35:39 E:27, Train sod-mae-score=0.0088-0.9866 gcn-mae-score=0.0438-0.9171 loss=304.3897(2128.6884+45.7604)
2020-09-07 03:35:39 E:27, Test  sod-mae-score=0.0390-0.8777 gcn-mae-score=0.0766-0.7417 loss=0.3422(0.1830+0.1592)

ckpt:./ckpt/PYG_GCNAtt_NoAddGCN_NoAttRes_Sigmoid_Abl2468/2
Total param: 64435137 lr_s=[[0, 5e-05], [20, 5e-06]]
2020-09-07 04:46:54 E:27, Train sod-mae-score=0.0088-0.9865 gcn-mae-score=0.0443-0.9168 loss=303.5541(2114.0333+46.0754)
2020-09-07 04:46:54 E:27, Test  sod-mae-score=0.0367-0.8838 gcn-mae-score=0.0752-0.7465 loss=0.3281(0.1811+0.1470)

ckpt:./ckpt/PYG_GCNAtt_NoAddGCN_NoAttRes_Sigmoid_Abl2468/3
Total param: 75346881 lr_s=[[0, 5e-05], [20, 5e-06]]
2020-09-07 07:18:11 E:29, Train sod-mae-score=0.0085-0.9869 gcn-mae-score=0.0415-0.9194 loss=292.9511(2045.1848+44.2163)
2020-09-07 07:18:11 E:29, Test  sod-mae-score=0.0363-0.8856 gcn-mae-score=0.0740-0.7471 loss=0.3338(0.1824+0.1515)
"""
...
if __name__ == '__main__':
    ...
    _root_ckpt_dir = "./ckpt/PYG_GCNAtt_NoAddGCN_NoAttRes_Sigmoid_Abl2468/{}".format(_gpu_id)
    ...
    runner = RunnerSPE(data_root_path=_data_root_path, root_ckpt_dir=_root_ckpt_dir,
                       sp_size=_sp_size, is_sgd=_is_sgd, lr=_lr, pool_num=_pool_num,
